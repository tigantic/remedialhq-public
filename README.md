...
Version `0.7.1` · release line, August 30, 2026
...
```bash
git clone ../remedialhq-engine-v0.7.1-public.bundle remedialhq-engine
...
The sanitized public repository has branch protection, secret scanning, push protection, dependency alerts, and automated security updates enabled. GitHub Actions run `33295512821` did not receive a runner and executed zero steps. GitHub reported an account-level billing entitlement message, which records a backend entitlement block and does not establish an unpaid balance. The v0.7.1 source line adds the replacement runtime, exact-image SBOM, independent Trivy and Grype gates, provenance and attestation verification, keyless signing, Google scan enforcement, and immutable-image smoke tests. Those remote workflow controls are recorded as unexecuted until GitHub allocates a runner, so deployment remains blocked.
