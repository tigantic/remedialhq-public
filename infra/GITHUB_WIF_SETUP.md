...
After both local scanners pass, the workflow waits for Google Artifact Analysis to report `FINISHED_SUCCESS`, a complete SBOM, completed operating-system and Python analysis, and zero findings whose effective severity is Critical or High. Findings for which Google publishes no effective severity are recorded as Unknown and are not silently promoted to a known severity. The read-only occurrence-viewer role cannot create or modify notes, occurrences, images, services, or IAM policy. Any CLI error, permission failure, malformed response, scan timeout, incomplete SBOM, or blocking finding stops the deployment.
