...
[[ "$PROJECT_NUMBER" =~ ^[0-9]+$ ]] || { echo "PROJECT_NUMBER is invalid." >&2; exit 4; }
...
ARTIFACT_ANALYSIS_SBOM_CONDITION="resource.name.startsWith('projects/_/buckets/${ARTIFACT_ANALYSIS_SBOM_BUCKET}/objects/')"
ARTIFACT_ANALYSIS_SBOM_CONDITION_TITLE="read-artifact-analysis-sboms"
ARTIFACT_ANALYSIS_SBOM_CONDITION_DESCRIPTION="Read only Artifact Analysis generated SBOM objects in this region."
gcloud projects remove-iam-policy-binding "$PROJECT_ID" \
  --member="serviceAccount:${DEPLOY_SA}" \
  --role="$ARTIFACT_ANALYSIS_SBOM_READER_ROLE" \
  --condition=None \
  --quiet >/dev/null 2>&1 || true
gcloud projects add-iam-policy-binding "$PROJECT_ID" \
  --member="serviceAccount:${DEPLOY_SA}" \
  --role="$ARTIFACT_ANALYSIS_SBOM_READER_ROLE" \
  --condition="expression=${ARTIFACT_ANALYSIS_SBOM_CONDITION},title=${ARTIFACT_ANALYSIS_SBOM_CONDITION_TITLE},description=${ARTIFACT_ANALYSIS_SBOM_CONDITION_DESCRIPTION}" \
  --quiet >/dev/null
...
while IFS= read -r role; do
  ...
  elif [[ "$role" == "$ARTIFACT_ANALYSIS_SBOM_READER_ROLE" ]]; then
    sbom_reader_role_found=true
  elif [[ -n "$role" ]]; then
...
if [[ "$sbom_reader_role_found" != true ]]; then
  echo "The conditional Artifact Analysis SBOM object-reader role was not bound to the GitHub deployer." >&2
  exit 5
fi
actual_sbom_reader_condition="$(gcloud projects get-iam-policy "$PROJECT_ID" \
  --flatten="bindings[].members" \
  --filter="bindings.role=${ARTIFACT_ANALYSIS_SBOM_READER_ROLE} AND bindings.members=serviceAccount:${DEPLOY_SA}" \
  --format="value(bindings.condition.expression)")"
actual_sbom_reader_title="$(gcloud projects get-iam-policy "$PROJECT_ID" \
  --flatten="bindings[].members" \
  --filter="bindings.role=${ARTIFACT_ANALYSIS_SBOM_READER_ROLE} AND bindings.members=serviceAccount:${DEPLOY_SA}" \
  --format="value(bindings.condition.title)")"
actual_sbom_reader_description="$(gcloud projects get-iam-policy "$PROJECT_ID" \
  --flatten="bindings[].members" \
  --filter="bindings.role=${ARTIFACT_ANALYSIS_SBOM_READER_ROLE} AND bindings.members=serviceAccount:${DEPLOY_SA}" \
  --format="value(bindings.condition.description)")"
if [[ "$actual_sbom_reader_condition" != "$ARTIFACT_ANALYSIS_SBOM_CONDITION" \
  || "$actual_sbom_reader_title" != "$ARTIFACT_ANALYSIS_SBOM_CONDITION_TITLE" \
  || "$actual_sbom_reader_description" != "$ARTIFACT_ANALYSIS_SBOM_CONDITION_DESCRIPTION" ]]; then
  echo "The GitHub deployer SBOM reader is not restricted to the Artifact Analysis bucket." >&2
  exit 5
fi
