...
EXPECTED_ARTIFACT_ANALYSIS_SBOM_CONDITION="resource.name.startsWith('projects/_/buckets/${EXPECTED_ARTIFACT_ANALYSIS_SBOM_BUCKET}/objects/')"
EXPECTED_ARTIFACT_ANALYSIS_SBOM_CONDITION_TITLE="read-artifact-analysis-sboms"
EXPECTED_ARTIFACT_ANALYSIS_SBOM_CONDITION_DESCRIPTION="Read only Artifact Analysis generated SBOM objects in this region."
...
while IFS= read -r role; do
  ...
  elif [[ "$role" == "$EXPECTED_CONTAINER_ANALYSIS_ROLE" ]]; then
    analysis_viewer_role_found=true
  elif [[ "$role" == "$EXPECTED_ARTIFACT_ANALYSIS_SBOM_READER_ROLE" ]]; then
...
if [[ "$analysis_viewer_role_found" != true ]]; then
  echo "The read-only Artifact Analysis occurrence role is not bound to the deploy service account." >&2
  echo "Run scripts/bootstrap_gcp.sh with the owner identity first." >&2
  exit 5
fi
if [[ "$sbom_reader_role_found" != true ]]; then
  echo "The conditional Artifact Analysis SBOM reader role is not bound to the deploy service account." >&2
  echo "Run scripts/bootstrap_gcp.sh with the owner identity first." >&2
  exit 5
fi
actual_sbom_reader_condition="$(gcloud projects get-iam-policy "$PROJECT_ID" \
  --flatten="bindings[].members" \
  --filter="bindings.role=${EXPECTED_ARTIFACT_ANALYSIS_SBOM_READER_ROLE} AND bindings.members=serviceAccount:${DEPLOY_SA}" \
...
if [[ "$actual_sbom_reader_condition" != "$EXPECTED_ARTIFACT_ANALYSIS_SBOM_CONDITION" \
  || "$actual_sbom_reader_title" != "$EXPECTED_ARTIFACT_ANALYSIS_SBOM_CONDITION_TITLE" \
  || "$actual_sbom_reader_description" != "$EXPECTED_ARTIFACT_ANALYSIS_SBOM_CONDITION_DESCRIPTION" ]]; then
  echo "Refusing WIF activation because the SBOM reader is not bucket-restricted." >&2
  echo "Run scripts/bootstrap_gcp.sh with the owner identity first." >&2
  exit 5
fi
