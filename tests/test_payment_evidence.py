...
def captured_document() -> dict[str, object]:
    return {
        ...
        "provider_purchase_sha256": f"{7:064x}",
...
class PaymentEvidenceTests(unittest.TestCase):
    def test_capture_returns_normalized_immutable_record_and_digest(self) -> None:
        ...
        self.assertEqual(record.to_dict()["tax_amount_cents"], 0)
        self.assertEqual(record.to_dict()["gross_amount_cents"], 9_900)
        self.assertEqual(record.provider_purchase_sha256, f"{7:064x}")
    ...
    def test_tax_inclusive_capture_and_full_gross_refund_share_provider_identity(
        self,
    ) -> None:
        capture = captured_document()
        capture["tax_amount_cents"] = 693
        capture["gross_amount_cents"] = 10_593
        refund = refund_document()
        refund["tax_amount_cents"] = 693
        refund["gross_amount_cents"] = 10_593
        refund["refunded_amount_cents"] = 10_593

        captured = build_live_payment_evidence(capture, expected_order_id=ORDER_ID)
        refunded = build_live_payment_evidence(refund, expected_order_id=ORDER_ID)

        self.assertEqual(captured.tax_amount_cents, 693)
        self.assertEqual(captured.gross_amount_cents, 10_593)
        self.assertEqual(refunded.refunded_amount_cents, 10_593)
        self.assertEqual(
            captured.provider_purchase_sha256,
            refunded.provider_purchase_sha256,
        )
    ...
    def test_exact_live_stripe_success_and_product_controls(self) -> None:
        invalid_values = (
            ...
            ("amount_cents", 99.0),
            ("tax_amount_cents", -1),
            ("tax_amount_cents", 1.0),
            ("gross_amount_cents", 9_901),
            ("provider_purchase_sha256", "A" * 64),
