...
def payment_evidence(
    ...
    *,
    ...
    document: dict[str, object] = {
        ...
        or f"{200 + number if refund else number:064x}",
    }
    if refund:
...
class PilotLedgerTests(unittest.TestCase):
    ...
    def purchase(self, prospect_id: str, number: int, *, fee_cents: int = 317) -> None:
        self.ledger.record(
            ...
            payment_evidence=payment_evidence(number),
        )
    ...
    def refund(self, prospect_id: str, number: int) -> None:
        self.ledger.record(
            ...
            payment_evidence=payment_evidence(number, refund=True),
        )
    ...
    def test_file_is_0600_and_generated_identifiers_are_opaque(self) -> None:
        ...
        self.assertEqual(initialized["payload"]["schema_version"], 4)
    ...
    def test_initialization_is_explicit_and_cannot_overwrite(self) -> None:
        ...
        with self.assertRaisesRegex(PilotReplayError, "initialize schema version 4"):
            PilotLedger(path)
        with self.assertRaises(PilotValidationError):
            PilotLedger.initialize(
                path,
                prior_consumed_slots=1,
                reconciliation_evidence_sha256=RECONCILIATION_EVIDENCE_SHA256,
            )
    ...
    def test_schema_four_ledger_can_roll_into_a_qualified_campaign(self) -> None:
    ...
    def test_zero_purchase_schema_four_rollover_keeps_the_audit_anchor(self) -> None:
        prospect_id = self.add_prospect(1)
        self.contact(prospect_id)
        ...
        initialized = json.loads(
            next_path.read_text(encoding="utf-8").splitlines()[0]
        )
        self.assertEqual(
    ...
    def test_permission_reporting_keeps_non_posix_mounts_usable(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            mounted_path = Path(directory) / "pilot.jsonl"
            mounted_ledger = PilotLedger.initialize(
                mounted_path,
                prior_consumed_slots=0,
                reconciliation_evidence_sha256=RECONCILIATION_EVIDENCE_SHA256,
            )
            expected = stat.S_IMODE(mounted_path.stat().st_mode) == 0o600
            self.assertEqual(mounted_ledger.private_mode_enforced, expected)
            self.assertEqual(mounted_ledger.metrics().prospects, 0)
    ...
    def test_invalid_funnel_transitions_are_rejected(self) -> None:
        ...
        with self.assertRaises(PilotValidationError):
            self.ledger.record(
                PilotEventType.SAMPLE_REQUESTED, {"prospect_id": prospect_id}
            )
        with self.assertRaises(PilotValidationError):
            self.ledger.record(
                PilotEventType.CHECKOUT_SENT,
    ...
    def test_rejected_order_allows_refund_suppression_and_bookkeeping(self) -> None:
        ...
        for event, payload in (
            ...
            with self.subTest(event=event), self.assertRaisesRegex(
                PilotValidationError, "allows only"
            ):
    ...
    def test_cancellation_blocks_new_and_continuing_fulfillment(self) -> None:
        ...
        with self.assertRaisesRegex(PilotValidationError, "blocked after cancellation"):
            self.ledger.record(
                PilotEventType.FULFILLMENT_STARTED, {"prospect_id": second}
            )
        ...
        for schema_version in (1, 2):
            with self.subTest(schema_version=schema_version):
                legacy_path = (
                    ...
                    / f"legacy-v{schema_version}.jsonl"
                )
    ...
    def test_schema_three_rejects_schema_four_events_and_relaxed_lineage(self) -> None:
        incompatible_path = (
            Path(self.temporary_directory.name) / "legacy-v3-incompatible.jsonl"
        )
