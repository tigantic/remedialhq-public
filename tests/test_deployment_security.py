...
class DeploymentSecurityTests(unittest.TestCase):
    ...
    def test_ci_project_roles_are_update_only_and_read_only_analysis(self) -> None:
        self.assertEqual(
            ...
            ('"$CI_RUN_ROLE"', '"$CONTAINER_ANALYSIS_VIEWER_ROLE"'),
        )
        self.assertIn(
    ...
    def test_google_artifact_analysis_is_a_fail_closed_deployment_gate(self) -> None:
        required = (
            ...
            'sbom_status.get("sbomState") != "COMPLETE"',
            '{"OS", "PYPI"}.issubset',
            'severity in {"CRITICAL", "HIGH"}',
            'vulnerability.get("effectiveSeverity", "UNKNOWN")',
            'severity_counts[severity] += 1',
            "Artifact Analysis did not finish with a complete SBOM in ten minutes",
