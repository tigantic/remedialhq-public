...
from remedialhq.youtube_upload_control import YouTubeUploadControlError
...
class PhaseTests(unittest.TestCase):
    ...
    def test_upload_control_rejection_becomes_a_phase_hold(self) -> None:
        root = Path(__file__).resolve().parents[1]
        with tempfile.TemporaryDirectory() as directory, patch.dict(
            os.environ,
            {
            ...
            "remedialhq.phases._publish_youtube",
            side_effect=YouTubeUploadControlError("confirmation already consumed"),
        ):
            result = run_phase("publish", root, directory)
        self.assertEqual(result.status, "HOLD")
        self.assertEqual(result.details["published"], [])
        self.assertIn(
            "YouTubeUploadControlError: confirmation already consumed",
            result.details["held"][0]["reason"],
        )
