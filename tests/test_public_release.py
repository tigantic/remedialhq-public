...
class PublicReleaseBuilderTests(unittest.TestCase):
    ...
    def test_post_validation_mode_is_a_closed_diff_allowlist(self) -> None:
        ...
        self._write(
            "artifacts/validation/analytics-javascript-syntax.txt",
            "new evidence\n",
        )
    ...
    def test_rejects_owner_identity_in_any_public_blob(self) -> None:
        ...
        cases = {
            ...
            "cloud-project-number": b"123456789012",
    ...
    def test_public_evidence_dependencies_and_private_validation_boundary(self) -> None:
        public_dependencies = {
            ...
            "src/remedialhq/pilot_reconciliation.py",
            "src/remedialhq/stripe_live_history.py",
            "src/remedialhq/stripe_reconciliation.py",
            "config/pilot-slot-reconciliation.schema.json",
            "scripts/check_google_artifact_analysis.py",
            "tests/test_payment_evidence.py",
            "tests/test_delivery_evidence.py",
            "tests/pilot_reconciliation_support.py",
            "tests/test_pilot_reconciliation.py",
            "tests/test_stripe_live_history.py",
            "tests/test_stripe_reconciliation.py",
            "tests/test_google_artifact_analysis.py",
        }
        private_validation = {
            "ops/CREATOR_DESK_INTAKE_TEMPLATE.md",
    ...
    def test_current_public_features_cross_only_exact_reviewed_paths(self) -> None:
        public_artifacts = {
            "artifacts/performance/calendar-reconciliation-2026-09-01.json",
        }
        public_sources = {
            "config/daily-performance.schema.json",
            "config/newsletter_contract.json",
            "ops/ANALYTICS_SETUP.md",
            "ops/NEWSLETTER_INTEGRATION.md",
            "ops/YOUTUBE_API_COMPLIANCE_EVIDENCE_MANIFEST.json",
            ...
            "src/remedialhq/daily_performance.py",
            "src/remedialhq/newsletter.py",
            "src/remedialhq/publishers/youtube.py",
            "src/remedialhq/youtube_verification.py",
            "tests/test_daily_performance.py",
        ...
        private_state = {
            ...
            "ops/execution_readiness.json",
        }

        self.assertTrue(public_artifacts <= release.PUBLIC_ARTIFACT_PATHS)
        self.assertTrue(public_artifacts | public_sources <= release.PUBLIC_FILE_PATHS)
        self.assertTrue(
            all(
                release._public_package_path(PurePosixPath(path))
                for path in public_artifacts | public_sources
            )
        )
        self.assertTrue(private_state <= release.PRIVATE_ADMIN_PATHS)
        self.assertTrue(private_state.isdisjoint({
            path
            for path in release.PUBLIC_FILE_PATHS
            if release._public_package_path(PurePosixPath(path))
        }))
        self.assertFalse(any(path.startswith("release/") for path in release.PUBLIC_FILE_PATHS))

        project_root = Path(__file__).resolve().parents[1]
        compliance_manifest = json.loads(
            (
                project_root / "ops/YOUTUBE_API_COMPLIANCE_EVIDENCE_MANIFEST.json"
            ).read_text(encoding="utf-8")
        )
        compliance_sources = {
            row["path"] for row in compliance_manifest["evidence_files"]
        }
        self.assertTrue(compliance_sources <= release.PUBLIC_FILE_PATHS)
        self.assertTrue(
            all(
                release._public_package_path(PurePosixPath(path))
                for path in compliance_sources
            )
        )
