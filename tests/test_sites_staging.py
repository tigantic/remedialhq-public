...
class SitesStagingTests(unittest.TestCase):
    def test_canonical_worker_redirects_provider_hosts_to_brand_only_domain(self) -> None:
        worker = (ROOT / "infra/sites/worker.ts").read_text(encoding="utf-8")

        self.assertIn('url.hostname.endsWith(".chatgpt.site")', worker)
        self.assertIn('url.hostname = "remedialhq.com"', worker)
        self.assertLess(
            worker.index('url.hostname.endsWith(".chatgpt.site")'),
            worker.index("const document = documents.get(url.pathname)"),
        )
    ...
    def test_stages_committed_documents_and_exact_public_asset_mirror(self) -> None:
        with tempfile.TemporaryDirectory(dir="/tmp") as directory:
            ...
            (target / "public/archive/nested.html").write_text("stale\n", encoding="utf-8")
            (target / "public/_redirects").write_text("stale\n", encoding="utf-8")
            (target / "worker/documents/legacy").mkdir(parents=True)
            (target / "worker/documents/legacy/nested.html").write_text("stale\n", encoding="utf-8")
            ...
            manifest = json.loads(
                (target / ".openai/remedialhq-staging-manifest.json").read_text(encoding="utf-8")
            )
            self.assertEqual(manifest["project_id"], "project_test")
            self.assertEqual(manifest["source_commit"], result["source_commit"])
            self.assertEqual(manifest["canonical_site_sha256"], result["canonical_site_sha256"])
            self.assertEqual(manifest["canonical_file_count"], result["canonical_file_count"])
            self.assertFalse(
    ...
    def test_write_failure_leaves_previous_destination_unchanged(self) -> None:
        with tempfile.TemporaryDirectory(dir="/tmp") as directory:
            ...
            with (
                mock.patch.object(staging, "_write_file", side_effect=interrupted_write),
    ...
    def test_swap_failure_rolls_back_both_trees_and_manifest(self) -> None:
        with tempfile.TemporaryDirectory(dir="/tmp") as directory:
            ...
            with (
                mock.patch.object(staging, "_replace_path", side_effect=interrupted_replace),
    ...
    def test_rejects_ignored_untracked_canonical_site_input(self) -> None:
        with tempfile.TemporaryDirectory(dir="/tmp") as directory:
            ...
            source = _make_source_repository(temporary_root)
            target = _make_destination(temporary_root)
            (source / ".gitignore").write_text("site/private.json\n", encoding="utf-8")
