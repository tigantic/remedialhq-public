...
class OutreachLedgerTests(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.suppression_evidence: dict[int, str] = {}
    ...
    def suppression_check(
        ...
        *,
        ...
        self.suppression_evidence[number] = evidence_sha256

    def contact(self, number: int, *, observed_at: str) -> None:
        evidence = ContactEvidence(
            prospect_id=opaque("prs", number),
            channel=ContactChannel.BUSINESS_EMAIL.value,
            sender_profile_evidence_sha256=digest(70_000),
            suppression_evidence_sha256=self.suppression_evidence.get(
                number,
                digest(10_000 + number),
            ),
            message_copy_sha256=digest(71_000 + number),
            provider_send_evidence_sha256=digest(72_000 + number),
            provider_message_sha256=digest(73_000 + number),
    ...
    def test_contact_requires_schedule_channel_and_fresh_suppression_check(self) -> None:
        ...
        with self.assertRaisesRegex(PilotValidationError, "post-send contact evidence"):
            self.ledger.record(
                PilotEventType.CONTACTED,
                {
        ...
        self.assertEqual(queue[0].contact_evidence_status, "VERIFIED")
        self.assertFalse(queue[0].contact_allowed)

    def test_contact_evidence_must_match_latest_suppression_and_be_promptly_recorded(self) -> None:
        self.import_plan()
        self.suppression_check(1, observed_at="2026-09-03T12:00:00Z")
        common = {
            "prospect_id": opaque("prs", 1),
            "channel": ContactChannel.BUSINESS_EMAIL.value,
            "sender_profile_evidence_sha256": digest(70_000),
            "message_copy_sha256": digest(71_001),
            "provider_send_evidence_sha256": digest(72_001),
            "provider_message_sha256": digest(73_001),
        }
        mismatched = ContactEvidence(
            **common,
            suppression_evidence_sha256=digest(99_999),
            observed_at="2026-09-03T12:01:00Z",
        )
        with self.assertRaisesRegex(PilotValidationError, "latest suppression"):
            self.ledger.record(
                PilotEventType.CONTACTED,
                {
                    "prospect_id": opaque("prs", 1),
                ...
                occurred_at="2026-09-03T12:01:00Z",
                contact_evidence=mismatched,
            )

        stale = ContactEvidence(
            **common,
            suppression_evidence_sha256=self.suppression_evidence[1],
            observed_at="2026-09-03T12:01:00Z",
        )
        with self.assertRaisesRegex(PilotValidationError, "within 15 minutes"):
            self.ledger.record(
                PilotEventType.CONTACTED,
                {
                    "prospect_id": opaque("prs", 1),
                ...
                occurred_at="2026-09-03T12:17:00Z",
                contact_evidence=stale,
            )
    ...
    def test_pre_contact_opt_out_can_be_replaced_without_expanding_the_cohort(self) -> None:
        ...
        self.ledger.record(
            PilotEventType.SUPPRESSION_CHECKED,
            {
                ...
                "evidence_observed_at": "2026-09-03T11:03:00Z",
            },
        ...
        contact_evidence = ContactEvidence(
            ...
            channel=ContactChannel.BUSINESS_EMAIL.value,
            sender_profile_evidence_sha256=digest(70_000),
            suppression_evidence_sha256=digest(50_003),
            message_copy_sha256=digest(71_099),
            provider_send_evidence_sha256=digest(72_099),
            provider_message_sha256=digest(73_099),
