...
PILOT_LEDGER_SCHEMA_VERSION: Final = 4
_PILOT_LEDGER_READABLE_SCHEMA_VERSIONS: Final = frozenset({3, 4})
...
ORDER_MANIFEST_SCHEMA_VERSION: Final = 4
_OPAQUE_ID_RE: Final = re.compile(
    r"^(?P<prefix>cmp|prs|chk|ord|pay|scp|cac|oac|orj|can|rfd|dlv|tim|inc|fbk)_"
...
class PilotLedger:
    ...
    def __init__(self, path: str | Path) -> None:
        ...
        if not self.path.exists():
            raise PilotReplayError(
                "pilot ledger does not exist; initialize schema version 4 first"
            )
    ...
    def _configure(self, path: str | Path) -> None:
        self.path = Path(path)
    ...
    def initialize(
        ...
        *,
        ...
        prior_ledger_head_sha256: str | None = None,
    ) -> PilotLedger:
        """Create a schema-4 ledger with an explicit lifetime-slot reconciliation."""
        instance = cls.__new__(cls)
        instance._configure(path)
        slots = cls._integer(
        ...
        prior_head = (
            None
            if prior_ledger_head_sha256 is None
            else cls._sha256_digest(
                prior_ledger_head_sha256,
                "prior_ledger_head_sha256",
            )
        )
        if slots > 0 and prior_head is None:
    ...
    @classmethod
    def reconcile_prior_ledger(cls, path: str | Path) -> tuple[int, str]:
        """Verify a prior ledger and conservatively count consumed founding slots."""
        source = Path(path)
        cls._assert_safe_ancestors(source)
        ...
        try:
            ...
            raise PilotReplayError("prior pilot ledger could not be opened") from exc
        try:
        ...
        schema_version = (
            ...
            if records[0].get("event_type")
            == PilotEventType.PILOT_LEDGER_INITIALIZED.value
        ...
        if schema_version in _PILOT_LEDGER_READABLE_SCHEMA_VERSIONS:
            ...
            with prior._guard, prior._process_lock():
                ...
                purchases = sum(
                    prospect.order_id is not None
                    for prospect in prior_state.prospects.values()
                )
                consumed_slots = prior_state.prior_consumed_slots + purchases
                if consumed_slots > FOUNDING_PURCHASE_LIMIT:
                    raise PilotReplayError(
                        "prior pilot ledger exceeds the five-slot limit"
                    )
                return consumed_slots, prior_state.head_sha256
        if schema_version not in {1, 2}:
            raise PilotReplayError(
                "prior pilot ledger must use schema version 1, 2, 3, or 4"
            )
        purchases = sum(
            record.get("event_type") == PilotEventType.PURCHASED.value
            for record in records
        )
        if purchases > FOUNDING_PURCHASE_LIMIT:
            raise PilotReplayError("prior pilot ledger exceeds the five-slot limit")
        return purchases, previous_hash
    ...
    def _bind_payment_evidence(
        ...
        expected_order_id = cls._opaque_id(
            data["order_id"], OpaqueIdKind.ORDER, "order_id"
        )
        try:
        ...
        if (
            validated.original_payment_ref is None
            or validated.refunded_amount_cents is None
        ):
        ...
        return {
            **data,
            ...
            "amount_cents": validated.refunded_amount_cents,
        }
    ...
    def _bind_delivery_evidence(
        ...
        if event is not PilotEventType.DELIVERED:
            if evidence is not None:
                raise PilotValidationError(
                    "delivery evidence is allowed only for a delivery event"
                )
            return payload
        if evidence is None:
            raise PilotValidationError(
                "delivery requires validated external delivery evidence"
            )
        ...
        return {
            **data,
            ...
            "delivery_evidence_artifact_sha256": (
                validated.evidence_artifact_sha256
            ),
    ...
    def outreach_queue(
        ...
        *,
        ...
        with self._guard, self._process_lock():
            ...
            active_prospect_ids = (
                entry.prospect_id for entry in state.outreach_plan.prospects
            )
            entries = (
    ...
    def order_at_head(self, order_id: str, ledger_head_sha256: str) -> PilotOrder:
        ...
        normalized_order_id = self._opaque_id(order_id, OpaqueIdKind.ORDER, "order_id")
        normalized_head = self._sha256_digest(
        ...
        for ancestor in reversed(absolute.parents):
            if ancestor.is_symlink():
                raise PilotLedgerError(
                    "owner-private paths must not use symbolic-link ancestors"
                )
    ...
    def _secure_file(self) -> None:
        ...
        try:
            ...
            raise PilotLedgerError("could not secure pilot ledger permissions") from exc
        finally:
            os.close(descriptor)
        self.private_mode_enforced = mode == 0o600
            ...
            if position == 0 and isinstance(record, dict):
                ...
                if legacy_schema in {1, 2}:
                    raise PilotReplayError(
                        ...
                        "initialize schema version 4 with lifetime-slot reconciliation"
                    )
            try:
                if not isinstance(record, dict) or set(record) != _LEDGER_KEYS:
    ...
    def _normalize_timestamp(value: object | None) -> datetime:
        ...
        try:
            ...
            raise PilotValidationError("occurred_at must be a timezone-aware ISO timestamp") from exc
    ...
    def _deadline(cls, value: object) -> str:
        ...
        if len(value) == 10:
            try:
                ...
                raise PilotValidationError("deadline must be an ISO date or aware timestamp") from exc
    ...
    def _apply_event(
        ...
        *,
        ...
        if event is PilotEventType.PILOT_LEDGER_INITIALIZED:
            ...
            if prior_consumed_slots > 0 and prior_ledger_head_sha256 is None:
                raise PilotValidationError(
                    "prior consumed slots require a prior ledger head digest"
                )
            if (
        ...
        if state.schema_version is None:
            raise PilotValidationError(
                "ledger initialization must be the first event"
            )
        if (
            state.schema_version == 3
            and event not in _PILOT_LEDGER_SCHEMA_3_EVENT_TYPES
        ):
            raise PilotValidationError(
        ...
        if event is PilotEventType.OUTREACH_PLAN_IMPORTED:
            ...
            if not plan.is_complete:
                raise PilotValidationError(
                    "outreach import requires a complete 50-prospect plan"
                )
        ...
        if event is PilotEventType.OUTREACH_PLAN_AMENDED:
            ...
            if current is None:
                raise PilotValidationError(
                    "an outreach plan must be imported before amendment"
                )
            ...
            amended_contract = {
                key: value
                for key, value in amended_document.items()
                if key != "prospects"
            }
            ...
            current_by_position = {
                item.queue_position: item for item in current.prospects
            }
            amended_by_position = {
                item.queue_position: item for item in plan.prospects
            }
            changed = [
            ...
            if len(changed) != 1:
                raise PilotValidationError(
                    "outreach amendment must replace exactly one prospect"
                )
            replaced_entry, replacement_entry = changed[0]
            if replaced_entry.prospect_id == replacement_entry.prospect_id:
                raise PilotValidationError(
                    "outreach amendment requires a new opaque prospect ID"
                )
            if (
                replaced_entry.planned_contact_date
                != replacement_entry.planned_contact_date
            ):
                raise PilotValidationError(
                    "replacement must retain the planned contact date"
                )
            ...
            if amended_local_date > date.fromisoformat(
                replaced_entry.planned_contact_date
            ):
                raise PilotValidationError(
        ...
        if event is PilotEventType.PROSPECT_ADDED:
            ...
            prospect_id = self._opaque_id(
                data["prospect_id"], OpaqueIdKind.PROSPECT, "prospect_id"
            )
        ...
        if event is PilotEventType.RISK_INCIDENT_RECORDED:
            ...
            incident_id = self._opaque_id(
                data["incident_id"], OpaqueIdKind.INCIDENT, "incident_id"
            )
        ...
        if (
            prospect.order_rejection_ref is not None
            and event
            not in {
                PilotEventType.REFUNDED,
                PilotEventType.OPTED_OUT,
        ...
        if event is PilotEventType.SUPPRESSION_CHECKED:
            ...
            if state.outreach_plan is None or prospect.queue_position is None:
                raise PilotValidationError(
                    "suppression checks require an imported outreach plan"
                )
        ...
        if event is PilotEventType.CONTACTED:
            data = self._fields(payload, {"prospect_id", "channel"})
            ...
            if state.outreach_plan is not None:
                ...
                contact_window_end = date.fromisoformat(
                    plan.campaign_start
                ) + timedelta(days=OUTREACH_WINDOW_END_DAY - 1)
                ...
                if contacts_today >= plan.daily_contact_limit:
                    raise PilotValidationError(
                        "daily contact limit has already been reached"
                    )
            prospect.contacted = True
            prospect.contacted_at = occurred_at
            return {"prospect_id": prospect_id, "channel": channel.value}
        ...
        if event is PilotEventType.SCOPE_AMENDED:
            ...
            if prospect.fulfillment_started:
                raise PilotValidationError(
                    "scope cannot be amended after fulfillment starts"
                )
            ...
            if supersedes_scope_ref != prospect.scope_ref:
                raise PilotValidationError(
                    "supersedes_scope_ref does not match the active scope"
                )
            ...
            if not isinstance(raw_claim_ids, list) or not 1 <= len(raw_claim_ids) <= 100:
                raise PilotValidationError("claim_ids must contain from 1 through 100 IDs")
            claim_ids = tuple(
                self._machine_safe(item, _CLAIM_ID_RE, "claim_ids")
        ...
        if event is PilotEventType.CUSTOMER_ACCEPTANCE_RECORDED:
            ...
            if scope_ref != prospect.scope_ref:
                raise PilotValidationError(
                    "customer acceptance does not match the active scope"
                )
        ...
        if event is PilotEventType.PURCHASED:
            ...
            if prospect.customer_acceptance_ref is None:
                raise PilotValidationError(
                    "purchase requires acceptance of the active scope"
                )
            if prospect.order_id is not None:
            ...
            payment_ref = self._opaque_id(
                data["payment_ref"], OpaqueIdKind.PAYMENT, "payment_ref"
            )
            payment_mode_value = self._enum(
                data["payment_mode"], PaymentMode, "payment_mode"
            )
            payment_mode = PaymentMode(payment_mode_value)
            if payment_mode is not PaymentMode.LIVE:
                raise PilotValidationError(
                    "TEST purchases cannot enter the live revenue ledger"
                )
            ...
            if payment_evidence_observed_at < prospect.checkout_occurred_at:
                raise PilotValidationError(
                    "payment evidence must be observed at or after checkout"
                )
            ...
            prospect.payment_evidence_artifact_sha256 = (
                payment_evidence_artifact_sha256
            )
            ...
            return {
                ...
                "payment_evidence_observed_at": (
                    payment_evidence_observed_at.isoformat()
                ),
        ...
        if event is PilotEventType.ORDER_ACCEPTED:
            ...
            if scope_ref != prospect.scope_ref:
                raise PilotValidationError(
                    "order acceptance does not match the active scope"
                )
            acceptance_ref = self._opaque_id(
        ...
        if event is PilotEventType.OPTED_OUT:
            ...
            if state.outreach_plan is not None and evidence_value is None:
                raise PilotValidationError(
                    "campaign opt-out requires a redacted evidence digest"
                )
        ...
        if event is PilotEventType.OWNER_TIME_RECORDED:
            data = self._fields(
                payload, {"prospect_id", "time_entry_id", "category", "minutes"}
            )
            ...
            if category is OwnerTimeCategory.FULFILLMENT:
                ...
                if not prospect.fulfillment_started:
                    raise PilotValidationError(
                        "fulfillment time requires fulfillment to start"
                    )
                if prospect.cancellation_ref is not None:
                    raise PilotValidationError(
                        "fulfillment time is blocked after cancellation"
                    )
                if prospect.refunded:
        ...
        if event is PilotEventType.FULFILLMENT_STARTED:
            ...
            if prospect.order_acceptance_ref is None:
                raise PilotValidationError(
                    "fulfillment requires post-payment order acceptance"
                )
            if prospect.cancellation_ref is not None:
        ...
        if event is PilotEventType.DELIVERED:
            ...
            if not prospect.artifact_completed or prospect.deliverable_sha256 is None:
                raise PilotValidationError(
                    "delivery requires a completed artifact"
                )
            if prospect.cancellation_ref is not None:
            ...
            prospect.delivery_evidence_artifact_sha256 = (
                delivery_evidence_artifact_sha256
            )
            ...
            return {
                ...
                "delivery_evidence_observed_at": (
                    delivery_evidence_observed_at.isoformat()
                ),
            }
        ...
        if event is PilotEventType.FEEDBACK_RECORDED:
            ...
            feedback_id = self._opaque_id(
                data["feedback_id"], OpaqueIdKind.FEEDBACK, "feedback_id"
            )
        ...
        if event is PilotEventType.REFUNDED:
            ...
            if original_payment_ref != prospect.payment_ref:
                raise PilotValidationError(
                    "refund evidence does not match the original payment"
                )
            refund_ref = self._opaque_id(
                data["refund_ref"], OpaqueIdKind.REFUND, "refund_ref"
            )
            ...
            prospect.refund_evidence_artifact_sha256 = (
                refund_evidence_artifact_sha256
            )
    ...
    def _suppression_is_fresh(
        ...
        if (
            prospect.suppression_status is not SuppressionStatus.CLEAR
    ...
    def _project_outreach_entry(
        ...
        return OutreachQueueEntry(
            ...
            contacted_at=(
                ...
                else prospect.contacted_at.isoformat()
            ),
    ...
    def _project_order(prospect_id: str, prospect: _ProspectState) -> PilotOrder:
        ...
        return PilotOrder(
            ...
            customer_acceptance_evidence_sha256=(
                prospect.customer_acceptance_evidence_sha256
            ),
            ...
            payment_evidence_observed_at=(
                prospect.payment_evidence_observed_at.isoformat()
            ),
            order_acceptance_ref=prospect.order_acceptance_ref,
            order_acceptance_evidence_sha256=(
                prospect.order_acceptance_evidence_sha256
            ),
    ...
    def _metrics(self, state: _PilotState) -> PilotMetrics:
        ...
        customer_acceptances = sum(
            item.customer_acceptance_ref is not None for item in prospects
        )
        ...
        orders_accepted = sum(
            item.order_acceptance_ref is not None for item in prospects
        )
        orders_rejected = sum(
            item.order_rejection_ref is not None for item in prospects
        )
        cancellation_requests = sum(
            item.cancellation_ref is not None for item in prospects
        )
        ...
        decision_gate = self._decision_gate(
            ...
            qualified_outreach_required=(
                state.schema_version == PILOT_LEDGER_SCHEMA_VERSION
            ),
        )
        return PilotMetrics(
            ...
            net_cash_cents=booked_revenue_cents
...
def _order_from_payload(payload: Mapping[str, object]) -> PilotOrder:
    ...
    prospect_id = PilotLedger._opaque_id(
        data["prospect_id"], OpaqueIdKind.PROSPECT, "prospect_id"
    )
    checkout_ref = PilotLedger._opaque_id(
        data["checkout_ref"], OpaqueIdKind.CHECKOUT, "checkout_ref"
    )
    checkout_occurred_at = PilotLedger._normalize_timestamp(
        data["checkout_occurred_at"]
    )
    order_id = PilotLedger._opaque_id(data["order_id"], OpaqueIdKind.ORDER, "order_id")
    payment_ref = PilotLedger._opaque_id(
        data["payment_ref"], OpaqueIdKind.PAYMENT, "payment_ref"
    )
    scope_ref = PilotLedger._opaque_id(
        data["scope_ref"], OpaqueIdKind.SCOPE, "scope_ref"
    )
    ...
    payment_mode_value = PilotLedger._enum(
        data["payment_mode"], PaymentMode, "payment_mode"
    )
    payment_mode = PaymentMode(payment_mode_value)
    if payment_mode is not PaymentMode.LIVE:
    ...
    if (order_acceptance_ref is None) != (
        order_acceptance_evidence_sha256 is None
    ):
        raise PilotValidationError(
    ...
    if not isinstance(raw_claim_ids, list) or not 1 <= len(raw_claim_ids) <= 100:
        raise PilotValidationError("claim_ids must contain from 1 through 100 IDs")
    claim_ids = tuple(
        PilotLedger._machine_safe(item, _CLAIM_ID_RE, "claim_ids")
        for item in raw_claim_ids
    )
    if len(set(claim_ids)) != len(claim_ids):
        raise PilotValidationError("claim_ids must be unique")
    ...
    if (deliverable_sha256 is None) != (artifact_completed_at is None):
        raise PilotValidationError(
            "completed artifact digest and timestamp must travel together"
        )
    ...
    delivery_evidence_artifact_value = data[
        "delivery_evidence_artifact_sha256"
    ]
    delivery_evidence_artifact_sha256 = (
        None
    ...
    if order_rejection_ref is not None and (
        ...
        raise PilotValidationError(
            "rejected order lifecycle fields are mutually exclusive"
        )
    ...
    if state is PilotOrderState.PURCHASED:
        ...
        if (
            ...
            raise PilotValidationError(
                "FULFILLMENT_STARTED manifest has invalid lifecycle fields"
            )
        ...
        if (
            ...
            raise PilotValidationError(
                "ARTIFACT_COMPLETED manifest has invalid lifecycle fields"
            )
    ...
    return PilotOrder(
        ...
        refund_evidence_observed_at=(
            None
            if refund_evidence_observed_at is None
            else refund_evidence_observed_at.isoformat()
        ),
        state=state,
        claim_ids=claim_ids,
        amount_cents=amount_cents,
        ...
        artifact_completed_at=(
            ...
            else artifact_completed_at.isoformat()
        ),
        delivery_ref=delivery_ref,
...
def _order_payload(order: PilotOrder) -> dict[str, object]:
    raw: dict[str, object] = {
        ...
        "refund_evidence_artifact_sha256": (
            order.refund_evidence_artifact_sha256
        ),
        ...
        "delivery_method": (
            None if order.delivery_method is None else order.delivery_method.value
        ),
        "delivery_evidence_sha256": order.delivery_evidence_sha256,
        "delivery_evidence_artifact_sha256": (
            order.delivery_evidence_artifact_sha256
        ),
    ...
    return {
        ...
        "refund_evidence_artifact_sha256": (
            normalized.refund_evidence_artifact_sha256
        ),
        ...
        "delivery_method": (
            None
            if normalized.delivery_method is None
            else normalized.delivery_method.value
        ),
        "delivery_evidence_sha256": normalized.delivery_evidence_sha256,
        "delivery_evidence_artifact_sha256": (
            normalized.delivery_evidence_artifact_sha256
        ),
    ...
    *,
    ...
    try:
        ...
        raise PilotManifestError(
            "order manifest must not use symbolic-link ancestors"
        ) from exc
    ledger_head = PilotLedger._sha256_digest(
        ledger_head_sha256, "ledger_head_sha256"
    )
...
def _decode_order_manifest(path: str | Path) -> tuple[PilotOrder, str]:
    ...
    try:
        ledger_head = PilotLedger._sha256_digest(
            raw["ledger_head_sha256"], "ledger_head_sha256"
        )
