...
class LivePaymentEvidence:
    ...
    def __post_init__(self) -> None:
        ...
        _match_string(
            self.provider_purchase_sha256,
            _SHA256_RE,
            "provider_purchase_sha256",
        )
        _match_string(self.artifact_sha256, _SHA256_RE, "artifact_sha256")
        _require_nonnegative_int(self.tax_amount_cents, "tax_amount_cents")
        _require_exact_int(
            self.gross_amount_cents,
            EXPECTED_AMOUNT_CENTS + self.tax_amount_cents,
            "gross_amount_cents",
        )
    ...
    def to_dict(self) -> dict[str, object]:
        record: dict[str, object] = {
            ...
            "provider_purchase_sha256": self.provider_purchase_sha256,
...
def build_live_payment_evidence(
    ...
    *,
    ...
    tax_amount_cents = _require_nonnegative_int(
        document["tax_amount_cents"],
        "tax_amount_cents",
    )
    gross_amount_cents = EXPECTED_AMOUNT_CENTS + tax_amount_cents
    _require_exact_int(
    ...
    provider_purchase_sha256 = _match_string(
        document["provider_purchase_sha256"],
        _SHA256_RE,
        "provider_purchase_sha256",
    )
...
def _require_nonnegative_int(value: object, field: str) -> int:
    if type(value) is not int or value < 0:
        raise PaymentEvidenceError(f"{field} must be a nonnegative integer")
    return value
